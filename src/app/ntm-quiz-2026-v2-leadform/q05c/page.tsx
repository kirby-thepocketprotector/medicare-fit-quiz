...
export default function Q05CPage() {
  ...
  const handleSelect = (choice: 'flexible' | 'lower_cost') => {
    ...
    // R08 and R09 should go directly to results (but these are already handled in q02a)
    // All other results (R01-R07) should go through contact gate
    if (result === 'R08' || result === 'R09') {
      router.push(`/ntm-quiz-2026-v2-leadform/result/${result}`);
    } else {
      // Route to contact page with result parameter
      router.push(`/ntm-quiz-2026-v2-leadform/contact?result=${result}`);
    }
  };
