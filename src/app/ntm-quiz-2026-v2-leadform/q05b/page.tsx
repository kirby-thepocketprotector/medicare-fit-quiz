...
import { trackViewBudgetChoiceAlex } from '@/utils/analytics';
...
export default function Q05BPage() {
  ...
  useEffect(() => {
    trackViewBudgetChoiceAlex();
  }, []);

  const handleContinue = () => {
    // If user has Medicaid, route to contact page for R03 (D-SNP)
    if (answers.hasMedicaid) {
      router.push('/ntm-quiz-2026-v2-leadform/contact?result=R03');
    } else {
      // Otherwise continue to final question
      router.push('/ntm-quiz-2026-v2-leadform/q05c');
    }
  };
    ...
    <div style={{ minHeight: '100vh', backgroundColor: Colors.background }}>
      ...
      <div style={{ maxWidth: '600px', margin: '0 auto', padding: '24px', paddingTop: '8px', paddingBottom: '100px' }}>
        ...
        <div style={{ marginTop: '8px' }}>
          <ContinueButton onPress={handleContinue} label="Let's See What Fits You Best" />
        </div>
      </div>
    </div>
  );
